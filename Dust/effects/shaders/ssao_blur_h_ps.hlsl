...
float4 main(float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_Target
{
    ...
    // mv = (sum_z, sum_z2, sum_ao, sum_ao_z) / 16
    float4 mv = 0;
    float4 ao, depth;
    ...
    // High relative variance (depth edges) -> large epsilon -> regresses toward local mean
    // Low relative variance (smooth surface) -> tiny epsilon -> preserves structure
